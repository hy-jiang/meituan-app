<template>
  <div class="s-item">
    ...
    <dd>
      ...
      <p>
        <el-rate v-model="meta.score" disabled show-score text-color="#ff9900" score-template=""></el-rate>
        <span v-if="meta.sore>4.5">很好</span>
        <span v-else-if="meta.sore>4">好</span>
        <span v-else-if="meta.sore>3.5">不错</span>
        <span v-else>一般</span>
      ...
      <ul class="deal-items">
        <li class="items" v-for="(item,index) in meta.dealItems.slice(0,2)" :key="index">
          ...
          <p class="">
            ...
            <span class="deal-salse">{{item.saleNum}}</span>
          </p>
        </li>
      </ul>
    </dd>
